...
- [4. Brincando com IO](#4-brincando-com-io)
  ...
  - [5.4 Funções e pure](#54-funções-e-pure)
...
Podemos separar nossas operações de IO em funções e ao invés de retornar `()` sempre, podemos retornar o tipo que quisermos. Além disso, podemos 
utilizar a função `pure` para "elevar" um valor normal a um IO exemplo `pure 3` que é do tipo `IO Int`:
