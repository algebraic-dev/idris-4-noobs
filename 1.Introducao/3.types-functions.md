...
- `()`, que pode ser `()`. Esse tipo representa uma unidade e por isso é chamado de Unit. Em muitos casos, ele é utilizado pra representar funções que não retornam nada significativo já que o valor sempre vai ser o mesmo.
...
Como pôde ver, é a mesma coisa que uma definição, porém, o tipo dela agora é o tipo de uma função que recebe `Int` e retorna `Int`, também, a definição inclui um nome a mais que é o `x` (após o nome `incrementar`) que nesse caso representa a nossa entrada e no final ela retorna o valor `x+1`. 

Para aplicarmos essa função, colocamos o valor que queremos que seja substituído no lugar de `x` após o nome da função como, por exemplo : `incrementar 3` que no final irá virar `4`. Se colocarmos o tipo errado (Já que a função pede um Int), a aplicação resultará em um erro.

```
Main> incrementar 3
4

Main> incrementar "esse é um texto"

Error: Can't find an implementation for FromString Int.

(Interactive):1:10--1:15
 1 | incrementar "esse é um texto"
                 ^^^^^^^^^^^^^^^^^
```
...
```idris
-- predecessor é a mesma coisa que receber x e retornar x - 1
predecessor : Int -> Int
predecessor = \x => x - 1
```
