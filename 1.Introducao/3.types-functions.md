...
Idris distingue entre funções *totais* e *parciais*. Funções totais são funções puras que conseguem terminar em tempo finito e reduzir o seu valor. Idris faz essa distinção para evitar que seja possível rodar funções infinitas em tempo de compilação (Veremos isso no capítulo 6!). Funções totais evitam falhas de comportamento inesperado por falta de atenção do programador as possibilidades, por esse motivo, é bem mais fácil trabalhar com funções totais em Idris do que com funções parciais.
 
Já Holes, servem para colocar um "buraco" no seu código em que podemos ver o tipo e também testar para ver se tudo ta correto antes de atingir esse ponto como, por exemplo, em:
...
Todo Hole começa com `?` e em seguida vem um nome que não deve estar previamente definido no programa. Quando formos ver o tipo desse hole no terminal ou utilizando o LSP, podemos ver também as outras variáveis.
