...
- `Char`, para uma único caractere, como `'b'`, por exemplo;
...
**Funções** são como máquinas que transformam o valor de entrada - também chamado de **parâmetro** - e retornam outros valores. Em Idris, todas as funções são puras, ou seja, é garantido que não vão modificar um valor que não esteja na função. Sendo assim, teoricamente, sempre irão retornar o mesmos valores a partir dos mesmos parâmetros.
